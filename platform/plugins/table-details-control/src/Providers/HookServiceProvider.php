...
class HookServiceProvider extends ServiceProvider
{
        ...
        if (($model instanceof Page || $model instanceof Post) && is_in_admin(true)) {
            if ($this->supportedRevisions($model)) {
                Assets::addStylesDirectly('vendor/core/packages/revision/css/revision.css')
                    ->addScriptsDirectly('vendor/core/packages/revision/js/html-diff.js');
            }
    ...
    public function addDetailsControlTabs(BaseModel $model, ?FormAbstract $form = null): void
    {
        if ($this->supportedRevisions($model)) {
            $this
                ->addTab(
        ...
        if ($form && is_plugin_active('language-advanced') && LanguageAdvancedManager::isSupported($model)) {
    ...
    public function addTab(string $id, string $label, Renderable|string $content, ?string $icon = null): void
    {
        add_filter(
    ...
    protected function supportedNote(BaseModel $model): bool
    {
    ...
    protected function supportedRevisions(BaseModel $model): bool
    {
