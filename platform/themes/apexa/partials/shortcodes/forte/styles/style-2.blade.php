@php
    $inlineVars = collect($variablesStyle ?? [])
        ->keys()
        ->implode(';');
@endphp
{{-- @dd($shortcode) --}}
<section id="forte" class="sc-forte forte-style-2 py-4 "
    @if ($inlineVars) style="{{ $inlineVars }}" @endif>
    <div class="container">
        <div class="row">
            {{-- LEFT --}}
            <div class="col-lg-6 my-auto">
                <div class="forte-content-left">
                    <div
                        class="section-title forte-content-left-title text-star mb-30 tg-heading-subheading animation-style3">
    ...
    @if (!empty($tabs_position))
        <div class="animation-icon-box">
            @foreach ($tabs_position as $d)
                ...
                @if (!empty($d['image']))
                    <div class="animation-icon {{ $animClass }}"
                        @if ($stylePlus) style="{{ $stylePlus }}" @endif>
